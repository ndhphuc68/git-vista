...
pub fn create_specta_builder() -> Builder<tauri::Wry> {
    Builder::<tauri::Wry>::new()
        ...
        // ("repo-changed-payload"), but these events are emitted by name through
        // `app.emit("repo-changed", ..)`, so that helper would listen on a channel
        // nothing sends to.
        .typ::<events::RepoChangedPayload>()
        .typ::<events::TaskProgressPayload>()
}
