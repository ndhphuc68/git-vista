...
fn test_get_commit_graph_merge_topology() {
    ...
    let c2 = repo.commit(Some("HEAD"), &sig, &sig, "commit 2 on master", &tree2, &[&c1]).unwrap();
    ...
    let c4 = repo.commit(Some("HEAD"), &sig, &sig, "commit 4 on master", &tree4, &[&c2_commit]).unwrap();
    ...
    let _c5 = repo.commit(Some("HEAD"), &sig, &sig, "commit 5 merge", &tree5, &[&c4_commit, &c3_commit]).unwrap();
    ...
    let has_merge_edge = result.commits.iter().any(|node| {
        node.lines.iter().any(|line| line.edge_type == "merge")
    });
    assert!(has_merge_edge, "Expected at least one merge edge when converging branches");

    // Root commit (c1) must not have zombie lines passing through
    let root_node = result.commits.last().unwrap();
    assert_eq!(root_node.lines.len(), 0, "Root commit must have no forward/pass-through lines");
}
