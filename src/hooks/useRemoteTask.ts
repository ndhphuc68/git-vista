...
interface TaskOwner { taskId: string; pending: boolean; cancelling: boolean; cancelPending: boolean }
...
export function useRemoteTask(repoPath: string | undefined, hasUpstream: boolean) {
  ...
  useEffect(() => {
    ...
    void listenToTaskProgress(payload => {
      ...
      setTask(previous => previous?.taskId === payload.task_id ? {
        ...previous,
  ...
  const clearCompletedTask = (owner: TaskOwner) => {
    ...
    clearTimer.current = setTimeout(() => {
      if (active.current === owner && !owner.cancelPending) {
        setTask(previous => previous?.taskId === owner.taskId && previous.status === "success" && !previous.cancelError ? null : previous);
      }
    }, 1000);
  };
  ...
  const run = async (operation: RemoteOperation) => {
    ...
    setTask({ taskId, operation, title: t.remoteProgress[`${operation}Title`], statusText: t.remoteProgress.starting, progressPercent: 0, status: "running" });
    try {
      if (operation === "fetch") await invokeCommand.fetchRepo(repoPath, undefined, false, taskId);
      else if (operation === "pull") await invokeCommand.pullRepo(repoPath, undefined, undefined, undefined, taskId);
      else await invokeCommand.pushRepo(repoPath, undefined, undefined, !hasUpstream, false, taskId);
      if (active.current !== owner) return;
      void queryClient.invalidateQueries();
      setTask(previous => previous?.taskId === taskId ? { ...previous, status: "success", statusText: t.remoteProgress.completed, progressPercent: 100 } : previous);
      clearCompletedTask(owner);
    } catch (error) {
      if (active.current !== owner) return;
      setTask(previous => previous?.taskId === taskId ? { ...previous, status: "error", error: mapGitError(error, t) } : previous);
      // Pull may leave conflicts even when it fails; refresh the affected views.
      void queryClient.invalidateQueries();
    } finally {
      owner.pending = false;
      if (active.current === owner) setTask(previous => previous?.taskId === taskId ? { ...previous, cancelling: false } : previous);
    }
  };
  ...
  const cancel = async (taskId: string) => {
    ...
    setTask(previous => previous?.taskId === taskId ? { ...previous, cancelling: true, cancelError: undefined, statusText: t.remoteProgress.cancelling } : previous);
    try {
      ...
      if (active.current !== owner) return;
      clearTimeout(clearTimer.current);
      setTask(previous => previous?.taskId === taskId ? { ...previous, cancelling: false, cancelError: mapGitError(error, t) } : previous);
    } finally {
  ...
  return {
    ...
    run,
    cancel,
    retry: () => { if (task?.operation && task.status === "error") void run(task.operation); },
    dismiss: () => { if (!active.current?.pending) { clearTimeout(clearTimer.current); setTask(null); } },
  };
}
