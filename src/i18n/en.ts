import { type Translations } from "./vi";

export const en: Translations = {
  appTitle: "GitVista",
  m0Badge: "Milestone M0: Foundation Shell",
  m1Badge: "Milestone M1: Git Viewer",
  screens: {
    welcome: "Welcome",
    changes: "Changes",
    history: "History",
    conflict: "Conflict Resolver",
  },
  gitActions: {
    simple: {
      fetch: "Check Remote Updates",
      pull: "Get Latest Changes",
      push: "Upload Changes",
      branch: "Working Branch",
      commit: "Save Changes",
      stage: "Mark for Saving",
      unstage: "Unmark",
      discard: "Discard Changes",
    },
    advanced: {
      fetch: "Fetch",
      pull: "Pull",
      push: "Push",
      branch: "Branch",
      commit: "Commit",
      stage: "Stage",
      unstage: "Unstage",
      discard: "Discard Changes",
    },
  },
  settings: {
    theme: "Theme",
    themeLight: "Light",
    themeDark: "Dark",
    themeSystem: "System Default",
    colorblind: "Colorblind Mode",
    colorblindOn: "Enabled",
    colorblindOff: "Disabled",
    language: "Language",
    mode: "Git Mode",
    modeSimple: "Simple (Friendly)",
    modeAdvanced: "Advanced (Standard Git)",
    title: "Settings",
    description: "Customize your Git profile, appearance, and application behavior",
    scopeSwitcher: {
      global: "Global App Settings",
      repo: "Repository",
      selectRepo: "Select Repository",
      noRepo: "No repo open",
    },
    tabs: {
      profile: "Git Profile",
      appearance: "Appearance & UX",
      diff: "Diff Viewer",
      behavior: "Git Behavior",
      tools: "External Tools",
      github: "GitHub",
    },
    profile: {
      title: "Git Author Profile",
      subtitle: "This information is attached to each commit you create.",
      scopeLabel: "Configuration Scope",
      scopeGlobal: "Global (~/.gitconfig)",
      scopeLocal: "This Repository (.git/config)",
      scopeGlobalDesc: "Applies to all repositories on your computer.",
      scopeLocalDesc: "Overrides settings specifically for this repository.",
      inheritedFromGlobal: "Inherited from Global",
      inheritGlobalOption: "Inherit configuration from Global",
      overrideRepoOption: "Override for this repo",
      resetToGlobalBtn: "Reset to Global",
      resetSuccess: "Reset to Global configuration successfully!",
      repoSettingsBanner: "Settings for repository:",
      repoSettingsDesc: "Override author profile and behavior specifically for this repository.",
      userNameLabel: "Author Name (user.name)",
      userNamePlaceholder: "e.g. Jane Doe",
      userEmailLabel: "Author Email (user.email)",
      userEmailPlaceholder: "e.g. jane@example.com",
      defaultBranchLabel: "Default Branch (init.defaultBranch)",
      defaultBranchPlaceholder: "main",
      saveBtn: "Save Git Profile",
      savedSuccess: "Git configuration saved successfully!",
      noRepoWarning: "No repository opened. Local configuration is disabled.",
      gpgTitle: "Commit GPG Signing",
      gpgDesc: "Automatically sign commits with a GPG key to verify author authenticity.",
      gpgEnable: "Sign all commits (commit.gpgsign)",
      gpgKeyLabel: "GPG Signing Key ID (user.signingkey)",
      gpgKeyPlaceholder: "e.g. 3AA5C34371567BD2",
      commitConventionsTitle: "Commit Message Conventions",
      commitLengthLabel: "Commit subject line length warning",
      commitLengthNoLimit: "No limit",
      commitLength50: "Warn when exceeding 50 chars (Git standard)",
      commitLength72: "Warn when exceeding 72 chars (GitHub standard)",
    },
    appearance: {
      title: "Appearance & Display",
      subtitle: "Personalize theme, language, and interface mode.",
      themeTitle: "Color Theme",
      themeLight: "Light",
      themeDark: "Dark",
      themeSystem: "System",
      localeTitle: "Display Language",
      localeVi: "Tiếng Việt",
      localeEn: "English",
      modeTitle: "Interface Mode",
      modeSimple: "Simple (Friendly)",
      modeSimpleDesc: "Streamlined UI focusing on essential daily Git actions.",
      modeAdvanced: "Advanced (Standard Git)",
      modeAdvancedDesc:
        "Full power with detailed graph, cherry-pick, rebase, and granular staging.",
      colorblindTitle: "Colorblind Accessibility",
      colorblindDesc: "Enhance contrast and color distinctiveness for color-impaired vision.",
      dateFormatTitle: "Date & Time Format",
      dateRelative: "Relative time (e.g. 2 hours ago, yesterday)",
      dateAbsolute: "Absolute time (e.g. 17/09/2026 14:30)",
      avatarTitle: "Author Avatar Style",
      avatarInitials: "Author Name Initials",
      avatarGravatar: "Gravatar (Loaded from author email)",
      avatarNone: "Hide avatar",
    },
    diff: {
      title: "Diff & Code Viewer",
      subtitle: "Customize layout, font size, and whitespace handling when comparing changes.",
      viewModeTitle: "Default Diff Layout",
      viewModeUnified: "Unified / Inline (Single column)",
      viewModeUnifiedDesc: "Display additions and deletions in an inline stream.",
      viewModeSplit: "Split / Side-by-side (Two columns)",
      viewModeSplitDesc: "Display original file on left and modified file on right.",
      fontSizeTitle: "Code Font Size",
      fontSize12: "Small (12px)",
      fontSize13: "Standard (13px)",
      fontSize14: "Medium (14px)",
      fontSize16: "Large (16px)",
      whitespaceTitle: "Whitespace Handling",
      whitespaceIgnore: "Ignore whitespace changes (--ignore-all-space)",
      whitespaceIgnoreDesc:
        "Do not highlight changes caused only by space or indentation adjustments.",
      tabSizeTitle: "Tab Indentation Width",
      tabSize2: "2 spaces",
      tabSize4: "4 spaces (Default)",
      tabSize8: "8 spaces",
      lineNumbersTitle: "Line Numbers",
      lineNumbersDesc: "Display line numbers before and after changes.",
    },
    behavior: {
      title: "Git Behavior Defaults",
      subtitle: "Configure how GitVista executes automated operations.",
      inheritGlobalPull: "Inherit from Global",
      inheritGlobalPullDesc: "Use system-wide default pull strategy ({strategy})",
      resetToGlobalBtn: "Reset to Global",
      resetSuccess: "Reset pull behavior to Global successfully!",
      pullRebaseTitle: "Pull Strategy",
      pullMerge: "Create Merge Commit (git pull --no-rebase)",
      pullMergeDesc: "Preserves branching history with merge commits.",
      pullRebase: "Rebase on top (git pull --rebase)",
      pullRebaseDesc: "Maintains a linear commit history without merge bubbles.",
      autoFetchTitle: "Background Auto-Fetch",
      autoFetchDesc: "Periodically check for remote updates in background.",
      autoFetchOff: "Off",
      autoFetch5m: "Every 5 minutes",
      autoFetch15m: "Every 15 minutes",
      fetchPruneTitle: "Prune Branches on Fetch (fetch.prune)",
      fetchPruneDesc:
        "Automatically remove remote-tracking references that no longer exist on remote.",
      rebaseAutostashTitle: "Auto-Stash during Rebase (rebase.autoStash)",
      rebaseAutostashDesc:
        "Automatically stash unstaged changes before rebasing and pop afterwards.",
      confirmationsTitle: "Confirmations & Safety Checks",
      confirmDiscardLabel: "Confirm before discarding changes",
      confirmDeleteBranchLabel: "Confirm before deleting a branch",
      confirmForcePushLabel: "Confirm before force pushing",
    },
    tools: {
      title: "External Tool Integrations",
      subtitle: "Configure your favorite code editor and command prompt.",
      editorTitle: "Default Code Editor / IDE",
      editorDesc: "Quickly open repository or source files in an external application.",
      editorVsCode: "Visual Studio Code (code)",
      editorCursor: "Cursor (cursor)",
      editorSublime: "Sublime Text (subl)",
      editorNotepadPlusPlus: "Notepad++ (notepad++)",
      editorCustom: "Custom CLI Command",
      editorCustomPlaceholder: "e.g. nvim, idea, or code-insiders",
      terminalTitle: "Default Terminal / Shell",
      terminalDesc: "Launch terminal window directly at the repository root directory.",
      terminalWindowsTerminal: "Windows Terminal (wt)",
      terminalPowerShell: "PowerShell (powershell)",
      terminalCmd: "Command Prompt (cmd)",
      terminalGitBash: "Git Bash (bash)",
    },
    help: {
      tagRecommended: "Recommended",
      tagWorkflow: "Workflow",
      tagSafety: "Safety",
      tagVisual: "Visual",
      tagIntegration: "Integration",
      pullStrategyTitle: "Git Pull Strategy",
      pullStrategyDesc:
        "Controls how incoming remote commits are integrated into your local branch. 'Rebase' maintains a clean linear history. 'Merge' preserves branch topology by creating a merge commit.",
      autoFetchTitle: "Background Fetch",
      autoFetchDesc:
        "Periodically checks the remote server for new commits silently without touching or interfering with your uncommitted working directory.",
      fetchPruneTitle: "Fetch Prune",
      fetchPruneDesc:
        "When a branch is deleted on GitHub/GitLab (e.g. after merging a PR), local tracking references remain. Prune automatically cleans up these dead references.",
      rebaseAutostashTitle: "Autostash on Rebase",
      rebaseAutostashDesc:
        "Automatically stashes uncommitted dirty changes before rebasing and pops them back after rebase finishes, avoiding stash conflicts.",
      confirmationsTitle: "Safety Confirmations",
      confirmationsDesc:
        "Shows confirmation dialogs before destructive actions (like discarding uncommitted changes or force pushing) to prevent irreversible data loss.",
      diffModeTitle: "Diff Display Mode",
      diffModeDesc:
        "Split (2 columns) allows side-by-side comparison on wide screens. Unified (1 column) shows changes inline, ideal for compact viewing.",
      diffWhitespaceTitle: "Ignore Whitespace in Diff",
      diffWhitespaceDesc:
        "Hides indentation and space changes so you only see actual logic and code modifications during review.",
      diffFontSizeTitle: "Diff Font Size",
      diffFontSizeDesc:
        "Adjusts code editor text size for optimal readability during diff inspection.",
      profileScopeTitle: "Config Scope (Global vs Repo)",
      profileScopeDesc:
        "Global applies across all repositories on your system. Repo overrides config only for the active project (useful for work vs personal emails).",
      profileGpgTitle: "GPG Commit Signing",
      profileGpgDesc:
        "Cryptographically signs commits with your GPG key to verify author identity (displays 'Verified' badge on GitHub).",
      appearanceThemeTitle: "Color Theme",
      appearanceThemeDesc:
        "Switch between Light, Dark, or System mode to match your desktop environment.",
      appearanceModeTitle: "UI Mode (Simple vs Advanced)",
      appearanceModeDesc:
        "Simple mode streamlines UI for clarity; Advanced mode unlocks deep Git tools like reflog, interactive rebase, and commit graphing.",
      toolsEditorTitle: "Default Code Editor",
      toolsEditorDesc:
        "Enables GitVista to open repositories and conflicted files directly in your preferred editor.",
      toolsTerminalTitle: "Default Terminal",
      toolsTerminalDesc:
        "Opens your preferred shell console directly in the repository root for advanced command-line operations.",
    },
    github: {
      title: "GitHub Integration",
      description: "Configure Personal Access Token to view and manage Pull Requests",
      tokenLabel: "GitHub Personal Access Token (PAT)",
      tokenPlaceholder: "ghp_...",
      tokenHelp: "Requires 'repo' scope (Full control of private repositories)",
      createTokenLink: "Generate new token on GitHub",
      testConnection: "Test Connection",
      testing: "Testing...",
      connectedAs: "Connected as",
      disconnect: "Disconnect",
      useGhCli: "Auto-detect from GitHub CLI (gh)",
      connectionSuccess: "GitHub connected successfully!",
      tokenSaved: "GitHub token saved successfully",
      tokenRemoved: "GitHub token removed",
      noToken: "No GitHub token configured",
    },
    close: "Close",
  },
  ipc: {
    testButton: "Test Rust IPC",
    pingResponse: "Response from Rust",
    systemInfo: "System Info",
    triggerEvent: "Simulate Repo Changed Event",
    lastEvent: "Latest Received Event",
    noEventsYet: "No events received from backend yet",
  },
  shell: {
    sidebarTitle: "Repos & Branches",
    graphTitle: "History Graph & Changes",
    detailTitle: "Details & Commit",
    emptyPlaceholder: "3-column layout shell ready for M1 (Git Viewer)",
    localBranches: "Local Branches",
    remoteBranches: "Remote Branches",
    stagedFiles: "Staged Files",
    unstagedFiles: "Unstaged Files",
  },
  welcome: {
    title: "Start with GitVista",
    tagline: "Fast, smooth, and beginner-friendly Git visualization.",
    openFolder: "Open Folder...",
    openFolderDesc: "Browse and open an existing local Git repository on your computer",
    cloneRepo: "Clone Repository...",
    cloneRepoDesc: "Download from GitHub, GitLab, or remote Git URL",
    dropzoneHint: "Drag and drop any Git folder here, or click to browse",
    recentTitle: "Recent Repositories",
    filterPlaceholder: "Filter repositories...",
    filterAria: "Filter recent repositories list",
    clearSearch: "Clear search",
    clearRecents: "Clean up",
    clearRecentsTooltip: "Clear all recent repositories history",
    loading: "Loading repositories...",
    emptyRecentsTitle: "No recent repositories yet.",
    emptyRecentsDesc: "Open a folder or Clone from remote to get started.",
    noMatch: 'No repositories found matching "{query}".',
    removeRecentTooltip: "Remove this repository from recent list",
    commandPalette: "Command Palette",
    shortcuts: "Shortcuts",
    gitMode: "Git Mode",
    errorOpen: "Cannot open repository. Please verify the folder path.",
    errorRecent: "Cannot open repository at: ",
    copyPath: "Copy path",
    copied: "Copied!",
    pin: "Pin to top",
    unpin: "Unpin",
    justNow: "Just now",
    minutesAgo: "{m}m ago",
    hoursAgo: "{h}h ago",
    daysAgo: "{d}d ago",
  },
  cloneModal: {
    title: "Clone Repository",
    desc: "Download complete repository history from remote to your computer",
    urlLabel: "Repository URL (Git URL)",
    urlPlaceholder: "https://github.com/user/repo.git or git@github.com:...",
    targetDirLabel: "Target Directory on disk",
    targetDirPlaceholder: "Path to destination folder...",
    selectFolder: "Browse folder",
    close: "Close",
    cancel: "Cancel",
    clone: "Clone",
    cloning: "Cloning...",
    initStatus: "Initializing clone...",
    cancelError: "Clone operation cancelled.",
    defaultError: "Cannot clone repository from this URL. Please verify.",
  },
  common: {
    ok: "OK",
    cancel: "Cancel",
    close: "Close",
    save: "Save",
    delete: "Delete",
    back: "Back",
    loading: "Loading...",
    search: "Search",
    clearSearch: "Clear search",
    copy: "Copy",
    copied: "Copied!",
    error: "Error",
    retry: "Retry",
    esc: "Esc",
    undo: "Undo",
  },
  sidebar: {
    branches: "BRANCHES",
    remotes: "REMOTES",
    tags: "TAGS",
    stashes: "STASHES",
    searchPlaceholder: "Filter branches...",
    searchAria: "Search branch",
    clearSearchAria: "Clear search",
    clearSearchTitle: "Clear search (Esc)",
    createBranchTitle: "Create new branch (Ctrl+B)",
    manageRemotesTitle: "Manage Remotes",
    addRemoteTitle: "Add remote...",
    pruneRemote: "Prune stale branches...",
    editRemote: "Edit remote...",
    removeRemote: "Remove remote...",
    emptyRemotes: "No remotes found.",
    emptyTags: "No tags found.",
    emptyStashes: "No stashes found.",
    applyStashTitle: "Apply stash",
    popStashTitle: "Apply & Pop stash",
    dropStashTitle: "Drop stash permanently",
    switchBranchSuccess: "Switched to branch {name}",
    toggleBranches: "Toggle branches list",
    toggleRemotes: "Toggle remotes list",
    toggleTags: "Toggle tags list",
    toggleStashes: "Toggle stashes list",
    checkoutBranch: "Checkout this branch",
    mergeIntoCurrent: "Merge into current branch",
    rebaseOntoThis: "Rebase current branch onto this",
    renameBranch: "Rename",
    deleteBranch: "Delete branch",
    createTagTitle: "Create new tag",
    checkoutTag: "Checkout this tag (Detached HEAD)",
    createBranchFromTag: "Create branch from this tag",
    pushTag: "Push tag to remote",
    deleteTag: "Delete tag",
    checkoutTagSuccess: "Checked out tag {name} (Detached HEAD)",
    pushTagSuccess: "Pushed tag {name} to remote successfully",
    compareWithCurrent: "Compare with current branch ({branch})...",
  },
  graph: {
    columns: {
      branchTag: "BRANCH / TAG",
      graph: "GRAPH",
      commitMessage: "COMMIT MESSAGE",
      author: "AUTHOR",
      sha: "SHA",
    },
    wipChanges: "Working directory has uncommitted changes",
    modifiedCount: "{n} modified",
    untrackedCount: "{n} new",
    justNow: "Just now",
    loadingMore: "Loading more commits...",
    allLoaded: "All {count} commits loaded",
    headBadge: "HEAD",
    createTagHere: "Create tag here",
    createBranchHere: "Create branch here",
    cherryPickHere: "Cherry-pick into current branch",
    revertHere: "Revert this commit",
    interactiveRebaseHere: "Interactive Rebase from here...",
    compareWith: "Compare with...",
    copySha: "Copy commit SHA",
    copyShaSuccess: "Copied commit SHA to clipboard",
  },
  diff: {
    ignoreWhitespace: "Ignore whitespace",
    ignoreWhitespaceActive: "Ignoring whitespace",
    wordDiff: "Word-level diff",
    wordDiffActive: "Word-level diff active",
    closeDetail: "Close",
    closeDetailAria: "Close commit details",
    closeDetailTitle: "Close commit details (Esc)",
    searchFilesPlaceholder: "Filter changed files...",
    clearFilterTitle: "Clear filter",
    noMatchingFiles: "No matching files found",
    authorTitle: "Commit author",
    filesChanged: "Files Changed ({count})",
    readingDiff: "Reading diff...",
    noDiffContent: "No content changes",
    binaryFile: "Binary file - cannot display diff",
    diffError: "Unable to read diff for this file.",
    noTextChanges: "No text changes in this file.",
    stagedBadge: "STAGED",
    unstagedBadge: "UNSTAGED",
    stageHunk: "Stage hunk",
    unstageHunk: "Unstage hunk",
    stageSelectedLines: "Stage selected lines",
    unstageSelectedLines: "Unstage selected lines",
    selectCommitPrompt: "Select a commit to view details and diff",
    loadingCommitDetails: "Loading commit details...",
    commitDetailsTitle: "Commit Details",
    copyShaTooltip: "Click to copy full SHA",
    commitTimeTitle: "Commit time",
    filesChangedHeader: "FILES CHANGED",
    filesCount: "{count} files",
    filesCountFiltered: "{filtered} / {total} files",
    fileLabel: "File:",
    copyPathTooltip: "Copy file path",
    prevFile: "Previous file",
    nextFile: "Next file",
    selectFilePrompt: "Select a file from the list to view diff",
    resizerTooltip: "Drag to resize file column (Double-click to reset 340px)",
    fileStatus: {
      added: "Added",
      deleted: "Deleted",
      renamed: "Renamed",
      modified: "Modified",
    },
    time: {
      justNow: "Just now",
      minutesAgo: "{m}m ago",
      hoursAgo: "{h}h ago",
      yesterday: "Yesterday",
      daysAgo: "{d}d ago",
      monthsAgo: "{mo}mo ago",
      yearsAgo: "{y}y ago",
    },
  },
  changes: {
    mobileTabFiles: "Files ({count})",
    mobileTabDiff: "Diff View",
    sidebarAria: "Changes list and commit box",
    diffViewerAria: "Diff viewer",
    noRepoOpen: "No repository opened",
    stagedTitle: "STAGED FILES",
    unstageAll: "Unstage All",
    noStagedFiles: "No staged files yet",
    conflictedTitle: "CONFLICTED FILES",
    resolveBtn: "Resolve",
    changesTitle: "CHANGES",
    stageAll: "Stage All",
    noChanges: "Working tree clean, no changes",
    unstageTitle: "Unstage",
    stageTitle: "Stage for commit",
    saveStashBtn: "Stash Changes",
    selectFileHint: "Select a file from the list to view diff and stage changes.",
    statusBadge: {
      conflicted: "Conflicted",
      modified: "Modified",
      untracked: "Untracked",
      deleted: "Deleted",
      renamed: "Renamed",
    },
  },
  commit: {
    title: "COMMIT",
    summaryPlaceholder: "Commit summary (concise, under 72 chars)...",
    charLimitWarn: "Exceeds 72 characters recommendation",
    descPlaceholder: "Extended description (optional)...",
    amendSimple: "Edit previous commit",
    amendAdvanced: "Amend (Edit last commit)",
    amendCommitSimple: "Update Previous Commit",
    amendCommitAdvanced: "Amend Commit",
    commitSimple: "Save Changes ({count} files)",
    commitAdvanced: "Commit ({count} files)",
    saving: "Saving...",
    commitSuccess: "Commit created successfully",
    amendSuccess: "Commit amended successfully",
    undo: "Undo",
  },
  discard: {
    success: "Discarded unstaged changes: {path}",
    title: "Discard Changes",
    description: "Changes in this file will be permanently discarded and cannot be recovered.",
    cancel: "Cancel",
    confirm: "Discard Changes",
  },
  conflictResolver: {
    loading: "Loading conflict data...",
    back: "Back",
    progress: "Conflicts: {resolved}/{total} hunks",
    prevConflict: "Previous Hunk",
    nextConflict: "Next Hunk",
    takeAllOurs: "Take All (Ours)",
    takeAllTheirs: "Take All (Theirs)",
    takeAllOursAria: "Take all Ours",
    takeAllTheirsAria: "Take all Theirs",
    oursHeader: "YOUR CHANGES (OURS)",
    mergedHeader: "MERGED RESULT",
    theirsHeader: "THEIR CHANGES (THEIRS)",
    acceptOurs: "Accept this change (Ours)",
    acceptTheirs: "Accept this change (Theirs)",
    complete: "Done",
    saving: "Saving...",
    takeBoth: "Take Both",
    selectedBadge: "✓ Resolved",
    unselectedBadge: "Unresolved",
    editorPlaceholder: "// Select ours/theirs or manually edit code here...",
    unresolvedWarning:
      "There are {count} unresolved conflict hunks remaining. Are you sure you want to save?",
  },
  modals: {
    createBranch: {
      title: "Create New Branch",
      fromCommit: "Based on commit:",
      nameLabel: "Branch Name",
      namePlaceholder: "e.g. feature/login, bugfix/navbar",
      checkoutLabel: "Checkout new branch after creation",
      cancel: "Cancel",
      creating: "Creating...",
      submit: "Create Branch",
      errorEmpty: "Please enter a branch name",
    },
    renameBranch: {
      title: "Rename Branch",
      currentLabel: "Current Branch Name",
      newLabel: "New Branch Name",
      cancel: "Cancel",
      renaming: "Renaming...",
      submit: "Rename Branch",
      errorEmpty: "Branch name cannot be empty",
      errorSame: "New branch name must be different from current name",
    },
    deleteBranch: {
      title: "Delete Branch",
      confirmMessage: "Are you sure you want to delete the following branch?",
      unmergedTitle: "Warning: Unmerged branch",
      unmergedWarning:
        "This branch contains commits not yet merged into HEAD. If you delete it, these commits will no longer appear on any branch.",
      backupNoticePrefix: "Safe: The system will automatically create a backup ref in",
      backupNoticeSuffix: "so you can restore it at any time within 30 days.",
      safeDelete: "Delete branch",
      forceDelete: "Delete this branch anyway",
      cancel: "Cancel",
      deleting: "Deleting...",
      successToast: "Deleted branch {name}",
      errorGeneric: "Failed to delete branch",
    },
    checkoutConflict: {
      title: "Checkout Conflict",
      description:
        "Cannot switch to branch {target} because you have uncommitted changes that would be overwritten.",
      advice:
        "To switch branches safely, stash your changes or switch to the Changes screen to commit them.",
      close: "Close",
      stashAndCheckout: "Stash & Checkout",
      stashingAndCheckout: "Stashing & switching...",
      toChanges: "Go to Changes",
      autoStashMessage: "Auto-stash before switching to {target}",
      stashError: "Error stashing & checking out: {msg}",
    },
    merge: {
      title: "Merge Branch",
      desc: "Merge changes from source branch into your current destination branch.",
      uncommittedWarn:
        "You have uncommitted changes. Stash or commit them before merging to avoid data loss.",
      noFfLabel: "No fast-forward (--no-ff)",
      cancel: "Cancel",
      merging: "Merging...",
      submit: "Merge Branch",
      conflictError: "Merge encountered conflicts. Please resolve conflicts before continuing.",
      genericError: "Merge failed: {msg}",
    },
    rebase: {
      title: "Rebase Branch",
      desc: "Re-apply commits from current branch on top of upstream branch.",
      uncommittedWarn: "Clean working tree required to rebase. Please stash or commit first.",
      cancel: "Cancel",
      rebasing: "Rebasing...",
      submit: "Rebase",
      conflictError: "Rebase encountered conflicts. Please resolve conflicts then click Continue.",
      genericError: "Rebase failed: {msg}",
    },
    createStash: {
      title: "Stash Changes",
      descLabel: "Description (optional)",
      descPlaceholder: "Stash description...",
      untrackedLabel: "Include untracked files",
      cancel: "Cancel",
      saving: "Stashing...",
      submit: "Stash Changes",
    },
    stashDiff: {
      applyBtn: "Apply",
      applyTitle: "Apply stash to working tree (keep stash)",
      popBtn: "Apply & Pop",
      popTitle: "Apply to working tree and remove stash",
      dropBtn: "Drop Stash",
      dropTitle: "Delete stash permanently",
      emptyFiles: "No files in this stash.",
      loadError: "Failed to load stash details.",
    },
    createTag: {
      title: "Create New Tag",
      targetCommit: "Target Commit:",
      nameLabel: "Tag Name",
      namePlaceholder: "e.g. v1.0.0, release-2026.09",
      annotatedLabel: "Annotated Tag",
      messageLabel: "Annotation Message",
      messagePlaceholder: "Enter release notes or description for this tag...",
      cancel: "Cancel",
      creating: "Creating...",
      submit: "Create Tag",
      errorEmpty: "Please enter a tag name",
      errorEmptyMessage: "Annotated tags require an annotation message",
      successToast: "Created tag {name}",
    },
    deleteTag: {
      title: "Delete Tag",
      confirmMessage: "Are you sure you want to delete the following tag?",
      targetCommit: "Points to commit:",
      deleteRemoteLabel: "Also delete this tag on remote 'origin'",
      cancel: "Cancel",
      deleting: "Deleting...",
      submit: "Delete Tag",
      successToast: "Deleted tag {name}",
      errorGeneric: "Failed to delete tag: {msg}",
    },
    cherryPick: {
      title: "Cherry-pick Commit",
      targetCommit: "Target Commit:",
      destinationBranch: "Destination Branch:",
      autoCommitLabel: "Auto-commit changes",
      autoCommitDesc: "Create commit immediately. If unchecked, changes will be staged.",
      cancel: "Cancel",
      submitting: "Cherry-picking...",
      submit: "Cherry-pick",
      conflictError: "Conflict occurred during cherry-pick. Please resolve conflicts.",
      conflictToast: "Conflict occurred during cherry-pick. Please resolve conflicts.",
      genericError: "Failed to cherry-pick: {msg}",
      successToast: "Successfully cherry-picked commit {commit}",
      stagedToast: "Changes staged. Review and commit when ready.",
    },
    revert: {
      title: "Revert Commit",
      desc: "This will create a new commit that inverts all changes from this commit without rewriting history.",
      targetCommit: "Target Commit:",
      autoCommitLabel: "Auto-commit changes",
      autoCommitDesc:
        "Create revert commit immediately. If unchecked, inverted changes will be staged.",
      cancel: "Cancel",
      submitting: "Reverting...",
      submit: "Revert Commit",
      conflictError: "Conflict occurred during revert. Please resolve conflicts.",
      conflictToast: "Conflict occurred during revert. Please resolve conflicts.",
      genericError: "Failed to revert commit: {msg}",
      successToast: "Successfully reverted commit {commit}",
      stagedToast: "Inverted changes staged.",
    },
    remotes: {
      title: "Manage Remotes",
      subtitle: "View, add, configure URLs, and prune remote Git repositories.",
      addRemoteBtn: "Add Remote",
      emptyTitle: "No remotes configured",
      emptyDesc:
        "Add a remote repository to synchronize code with GitHub, GitLab, or your private server.",
      fetchUrlLabel: "Fetch URL",
      pushUrlLabel: "Push URL",
      branchCount: "{count} branches",
      defaultBadge: "Default",
      copyUrlTooltip: "Copy URL",
      copiedToast: "Remote URL copied to clipboard",
      actions: {
        prune: "Prune",
        pruneTooltip: "Delete remote-tracking branches that have been removed on the server",
        edit: "Edit",
        delete: "Delete",
      },
      addModal: {
        title: "Add Remote Repository",
        editTitle: "Edit Remote Repository",
        nameLabel: "Remote Name",
        namePlaceholder: "e.g. origin, upstream",
        fetchUrlLabel: "Fetch URL",
        fetchUrlPlaceholder: "https://github.com/user/repo.git or git@github.com:...",
        separatePushUrl: "Use a different URL for push",
        pushUrlLabel: "Push URL",
        pushUrlPlaceholder: "Leave blank if same as Fetch URL",
        submitAdd: "Add Remote",
        submitEdit: "Save Changes",
        saving: "Saving...",
        errorNameEmpty: "Remote name cannot be empty",
        errorNameInvalid: "Remote name cannot contain special characters or spaces",
        errorUrlEmpty: "Fetch URL cannot be empty",
        addSuccess: "Remote {name} added successfully",
        editSuccess: "Remote {name} updated successfully",
      },
      deleteModal: {
        title: "Delete Remote Repository",
        confirmMessage:
          "Are you sure you want to remove the following remote from this repository?",
        warning:
          "This only removes the remote configuration and its local tracking branches. It does not affect data on the remote server.",
        confirmBtn: "Delete Remote",
        deleting: "Deleting...",
        successToast: "Remote {name} deleted",
      },
      pruneModal: {
        title: "Prune Stale Remote Branches",
        confirmMessage:
          "Pruning will scan the remote repository and delete local remote-tracking branches that no longer exist on the server.",
        safeNotice: "Safe: Your local personal branches will not be affected at all.",
        confirmBtn: "Prune Now",
        pruning: "Pruning...",
        successToast: "Prune completed: {count} stale branches removed.",
        nonePrunedToast: "All remote branches are up to date, no stale branches to prune.",
      },
    },
    interactiveRebase: {
      title: "Visual Interactive Rebase",
      subtitle: "Rebase commits onto base {base}",
      loadingCommits: "Loading commits...",
      noCommits: "No commits found to rebase between this base and HEAD.",
      moveUpTooltip: "Move up (execute earlier)",
      moveDownTooltip: "Move down (execute later)",
      dragHandleTooltip: "Drag to reorder commit",
      editMessagePlaceholder: "Enter new commit message...",
      actions: {
        pick: "Pick",
        pickDesc: "Keep commit as is",
        reword: "Reword",
        rewordDesc: "Keep commit but change commit message",
        squash: "Squash",
        squashDesc: "Meld into previous commit & combine messages",
        fixup: "Fixup",
        fixupDesc: "Meld into previous commit & discard message",
        drop: "Drop",
        dropDesc: "Remove commit from history",
      },
      validation: {
        firstCannotSquash:
          "The first commit cannot be squashed or fixed up (no previous commit to meld into).",
        cannotDropAll: "Cannot drop all commits in this rebase.",
        emptyMessage: "Commit message cannot be empty when rewording or squashing.",
        dirtyWorkingTree:
          "Working tree has uncommitted changes. Please stash or enable Auto-stash.",
      },
      preview: {
        title: "Live Preview",
        resultingCommits: "{count} resulting commit(s)",
        squashedCount: "{count} squashed",
        droppedCount: "{count} dropped",
        rewordedCount: "{count} reworded",
        noChangesNotice: "Commit sequence and messages currently unchanged.",
      },
      autoStash: "Automatically stash uncommitted changes (Auto-stash)",
      resetBtn: "Reset to Original",
      cancelBtn: "Cancel",
      startBtn: "Start Rebase",
      rebasing: "Rebasing...",
      successToast: "Interactive Rebase succeeded! HEAD updated.",
      conflictToast:
        "Rebase encountered conflicts. Please resolve conflicts in the Conflict Resolver.",
      errorToast: "Interactive Rebase failed: {msg}",
      undoSuccessToast: "Interactive Rebase undone! Branch restored to original state.",
    },
  },
  banner: {
    inProgress: "Operation in progress: {operation}",
    branchLabel: "(branch {name})",
    conflictsCount: "{count} conflicted files",
    noConflicts: "No conflicted files",
    viewChanges: "View Conflicted Files",
    continueOp: "Continue",
    abortOp: "Abort",
    continuing: "Continuing...",
    aborting: "Aborting...",
    abortError: "Failed to abort: {msg}",
    continueError: "Failed to continue: {msg}",
    continueTooltipDisabled: "Please resolve all conflicts before continuing",
    continueTooltip: "Continue operation",
  },
  remoteProgress: {
    cancelFailed: "Cancellation failed",
    cancelling: "Cancelling…",
    completed: "Completed",
    copied: "Details copied",
    copyDetails: "Copy technical details",
    copyFailed: "Could not copy. Select and copy the technical details below.",
    dismiss: "Dismiss",
    genericHint:
      "Review the technical details, check the repository and remote settings, then retry.",
    retry: "Retry",
    stillRunning: "The operation is still running. Wait for it to finish or try cancelling again.",
    technicalDetails: "Technical details",
    cancelTask: "Cancel",
    cancelTaskAria: "Cancel task",
    fetchTitle: "Fetching from remote",
    pullTitle: "Pulling from remote",
    pushTitle: "Pushing to remote",
    starting: "Starting...",
  },
  header: {
    toggleSidebar: "Toggle sidebar ({shortcut})",
    switchRepo: "Switch repository",
    repoBtn: "Repo",
    refreshRepo: "Refresh repository",
    settingsTitle: "Settings (Theme, Language, Git Mode)",
    settingsMenuTitle: "System Settings",
    commitsBehind: "{count} commits to pull",
    commitsAhead: "{count} commits to push",
    pushUpstream: "Push and set upstream to remote",
    pushNormal: "Push commits to remote",
    fetchTitle: "Fetch from remote",
    pullTitle: "Pull latest commits from remote",
  },
  palette: {
    categories: {
      navigation: "Navigation",
      branch: "Branch",
      git: "Git Actions",
      settings: "Settings & Help",
    },
    commands: {
      navHistoryTitle: "Go to Commit History",
      navHistoryDesc: "Switch to commit history and branch tree view",
      navChangesTitle: "View Working Changes",
      navChangesDesc: "Switch to changes management and commit screen",
      branchCreateTitle: "Create New Branch",
      branchCreateDesc: "Open dialog to create a new Git branch",
      gitFetchTitle: "Fetch from Remote",
      gitFetchDesc: "Fetch latest references from remote server",
      gitPullTitle: "Pull Changes",
      gitPullDesc: "Fetch and merge new commits into current branch",
      gitPushTitle: "Push Commits",
      gitPushDesc: "Push local commits to remote repository",
      gitStageAllTitle: "Stage All Changes",
      gitStageAllDesc: "Stage all modified and untracked files for commit",
      gitCommitTitle: "Create Commit",
      gitCommitDesc: "Save staged changes as a new commit",
      gitManageRemotesTitle: "Manage Remotes",
      gitManageRemotesDesc: "View, add, edit URLs, or prune remote repositories",
      gitPruneRemotesTitle: "Prune Stale Remote Branches",
      gitPruneRemotesDesc: "Remove remote-tracking branches that were deleted on the server",
      gitInteractiveRebaseTitle: "Git: Visual Interactive Rebase",
      gitInteractiveRebaseDesc:
        "Open visual interactive rebase to reorder, pick, reword, squash, or drop commits",
      gitCompareTitle: "Git: Compare 2 Commits or Branches",
      gitCompareDesc: "Open visual compare dialog to view diff and commits between two revisions",
      gitCreatePrTitle: "Git: Create New Pull Request",
      gitCreatePrDesc: "Open modal to create a new Pull Request on GitHub",
      gitViewPrsTitle: "Git: View Pull Requests",
      gitViewPrsDesc: "View GitHub Pull Requests in sidebar drawer",
      settingsThemeTitle: "Toggle Light / Dark Theme",
      settingsThemeDesc: "Switch theme between dark and light modes",
      settingsModeTitle: "Toggle Git Mode",
      settingsModeDesc: "Switch between Simple and Advanced Git modes",
      settingsShortcutsTitle: "Keyboard Shortcuts",
      settingsShortcutsDesc: "View list of all application keyboard shortcuts",
    },
    inputPlaceholder: "Type a command or search actions...",
    emptyMatch: 'No commands found matching "{query}".',
    navigateHint: "Navigate",
    executeHint: "Execute",
    closeHint: "Close",
    commandsCount: "{count} commands",
    clear: "Clear",
  },
  shortcuts: {
    title: "Keyboard Shortcuts",
    subtitle: "Quick key combinations for faster navigation",
    categories: {
      general: "General",
      navigation: "Screen Navigation",
      git: "Git Operations",
      settings: "Settings & Appearance",
    },
    items: {
      commandPalette: "Command Palette",
      shortcutsHelp: "Shortcuts Help",
      closeModal: "Close Modal / Escape",
      historyScreen: "History Screen",
      changesScreen: "Changes Screen",
      createBranch: "Create New Branch",
      commitChanges: "Save Changes (Commit)",
      fetchRemote: "Fetch from Remote",
      pullRemote: "Pull from Remote",
      pushRemote: "Push to Remote",
      stageAll: "Stage All Changes",
      toggleTheme: "Toggle Light/Dark Theme",
    },
    tip: "Tip: Press {key} anytime to open this helper",
    escToClose: "Esc to close",
  },
  toast: {
    technicalDetails: "Technical Details",
    undoing: "Undoing...",
    undo: "Undo",
    close: "Close",
  },
  errors: {
    authFailedTitle: "Git Authentication Failed",
    authFailedMessage: "Unable to connect or permission denied for remote Git server.",
    authFailedHint: "Please check your SSH keys or Personal Access Token.",
    remoteNewCommitsTitle: "Remote Contains New Commits",
    remoteNewCommitsMessage: "The remote server has commits that do not exist locally.",
    remoteNewCommitsHint: "Please Pull latest changes before Pushing.",
    checkoutConflictTitle: "Checkout Conflict",
    checkoutConflictMessage: "You have uncommitted changes that would be overwritten.",
    checkoutConflictHint: "Commit or Stash your changes before switching branches.",
    networkErrorTitle: "Network Error",
    networkErrorMessage: "Unable to communicate with the remote server.",
    networkErrorHint: "Please check your internet connection and try again.",
    genericTitle: "Operation Failed",
  },
  shellExt: {
    resizeTooltip: "Drag to resize sidebar (Double click to reset)",
  },
  inspector: {
    title: "File Inspector",
    blameTab: "Git Blame",
    historyTab: "File History",
    copyPath: "Copy path",
    copyPathSuccess: "File path copied to clipboard",
    copyShaSuccess: "Commit SHA copied to clipboard",
    closeTooltip: "Close (Esc)",
    loadingBlame: "Loading Blame data...",
    loadingHistory: "Loading file commit history...",
    noBlameData: "No blame data available for this file.",
    noHistoryData: "No commit history found for this file.",
    searchHistoryPlaceholder: "Filter commits by message or author...",
    noMatchingCommits: "No commits match the filter.",
    totalLines: "{count} lines",
    totalCommits: "{count} commits",
    changeTypeAdded: "Added",
    changeTypeModified: "Modified",
    changeTypeDeleted: "Deleted",
    jumpToCommit: "View commit on graph",
    viewBlame: "View Git Blame",
    viewHistory: "View File History",
  },
  compare: {
    title: "Compare Commits & Branches",
    base: "Base",
    target: "Compare (Target)",
    swap: "Swap base and target",
    modeMergeBase: "Merge-Base (A...B)",
    modeMergeBaseDesc: "Changes since diverging from common ancestor (Pull Request style)",
    modeDirect: "Direct (A..B)",
    modeDirectDesc: "Direct tree-to-tree comparison between two revision tips",
    tabCommits: "Commits ({count})",
    tabFiles: "Changed Files ({count})",
    searchFiles: "Search changed files...",
    noCommits: "No differing commits between revisions",
    noFiles: "No changed files found",
    identical: "The two revisions are identical",
    aheadBehind: "{ahead} commits ahead, {behind} commits behind",
    stats: "+{additions} lines / -{deletions} lines in {files} files",
    selectFileToViewDiff: "Select a file on the left to view diff",
    close: "Close",
    openInCompare: "Compare",
  },
  pullRequests: {
    title: "Pull Requests",
    open: "Open",
    mine: "Mine",
    closed: "Closed",
    newPr: "Create Pull Request",
    refresh: "Refresh list",
    noPrs: "No Pull Requests found",
    notGitHub: "This repository is not hosted on GitHub",
    checkout: "Checkout PR Branch",
    checkingOut: "Switching branch...",
    checkoutSuccess: "Switched to branch {branch} successfully",
    openInBrowser: "Open on GitHub.com",
    copyLink: "Copy PR Link",
    linkCopied: "PR link copied",
    ciStatus: "CI Status",
    ciPassed: "All checks have passed",
    ciFailed: "Checks failed",
    ciRunning: "Checks in progress...",
    filesChanged: "Files Changed ({count})",
    description: "Description",
    noDescription: "No description provided for this Pull Request.",
    reviewers: "Reviewers",
    labels: "Labels",
    offline: "Unable to connect to GitHub. Please check your network connection.",
    rateLimit: "GitHub API rate limit exceeded. Please configure a Token in Settings.",
    needToken: "GitHub Token required in Settings",
    createModalTitle: "Create Pull Request",
    baseBranch: "Base Branch",
    compareBranch: "Compare Branch",
    prTitle: "Pull Request Title",
    prTitlePlaceholder: "Enter a concise summary of changes...",
    prBody: "Description (Markdown supported)",
    prBodyPlaceholder: "Describe your changes, fixes, or testing notes...",
    isDraft: "Create as draft (Draft PR)",
    submitCreate: "Create Pull Request",
    creating: "Creating...",
    unpushedWarning: "This branch has unpushed commits that are not yet on GitHub",
    pushFirst: "Push to GitHub first",
    createSuccess: "Pull Request #{number} created successfully!",
  },
};
