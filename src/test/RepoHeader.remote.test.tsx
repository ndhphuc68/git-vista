...
describe("RepoHeader remote task lifecycle", () => {
  beforeEach(() => {
    ...
    useRepoStore.getState().setRepo({ path: "/repo", name: "repo", is_bare: false, head_branch: "main", head_commit_id: "abc" });
    vi.spyOn(ipc, "listenToTaskProgress").mockResolvedValue(vi.fn());
  });
  afterEach(() => { vi.useRealTimers(); vi.restoreAllMocks(); });

  it.each(["fetch", "pull", "push"] as const)("keeps %s failures visible and allows retry and dismissal", async operation => {
    const command = vi.spyOn(ipc.invokeCommand, `${operation}Repo`).mockRejectedValue(new Error("Authentication failed"));
    showHeader();
    await act(async () => { fireEvent.click(screen.getByTestId(`btn-${operation}`)); });
  ...
  it("does not let a previous success timer clear a newer running task", async () => {
    ...
    showHeader();
    await act(async () => { fireEvent.click(screen.getByTestId("btn-fetch")); });
    await act(async () => { fireEvent.click(screen.getByTestId("btn-pull")); });
    await act(async () => { vi.advanceTimersByTime(1500); });
    expect(screen.getByRole("progressbar")).toBeInTheDocument();
    expect(screen.getByTestId("btn-fetch")).toBeDisabled();
    await act(async () => { pull.resolve("ok"); });
  });
  ...
  it("keeps operations busy after cancellation request until the task settles", async () => {
    ...
    await act(async () => { fetch.reject(new Error("Operation cancelled")); });
  ...
  it("shows cancellation failure while keeping the operation busy", async () => {
    const fetch = deferred<string>();
    vi.spyOn(ipc.invokeCommand, "fetchRepo").mockReturnValue(fetch.promise);
    vi.spyOn(ipc.invokeCommand, "cancelRemoteTask").mockRejectedValue(new Error("Cancel unavailable"));
    showHeader();
    await act(async () => { fireEvent.click(screen.getByTestId("btn-fetch")); });
    await act(async () => { fireEvent.click(screen.getByRole("button", { name: "Cancel task" })); });
    expect(screen.getByRole("alert")).toHaveTextContent("Cancellation failed");
    expect(screen.getByTestId("btn-push")).toBeDisabled();
    await act(async () => { fetch.resolve("ok"); });
  });
  ...
  it("ignores old task completion after switching repositories", async () => {
    ...
    showHeader();
    await act(async () => { fireEvent.click(screen.getByTestId("btn-fetch")); });
    act(() => useRepoStore.getState().setRepo({ path: "/other", name: "other", is_bare: false, head_branch: "main", head_commit_id: "abc" }));
    expect(screen.queryByRole("progressbar")).not.toBeInTheDocument();
    await act(async () => { fetch.reject(new Error("Old repo failure")); });
    expect(screen.queryByRole("alert")).not.toBeInTheDocument();
  ...
  it("retains a cancellation error even when cancellation fails after task completion", async () => {
    ...
    await act(async () => { fetch.resolve("ok"); });
    await act(async () => { vi.advanceTimersByTime(1500); });
    await act(async () => { cancel.reject(new Error("Cancel unavailable")); });
    expect(screen.getByRole("alert")).toHaveTextContent("Cancellation failed");
    expect(screen.getByTestId("btn-fetch")).toBeEnabled();
  });
});
